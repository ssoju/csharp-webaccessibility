...
// CHtmlParser.cs: implementation of the CHtmlParser class.
//
///////////////////////////////////////////////////////////////////////////////
...
namespace Cloud9.Parser.Html
{
	...
	/// </summary>
    public sealed class CHtmlParser : IHtmlParser
	{
    /////////////////////////////////////////////////////////////////////////////////
    #region 資?結構
    ...
    #region 解析 Token
        ...
        private void ResolveStyle(ref int visitor, ref int nodeIDCount)
        {
            ...
            CHtmlStyle style = new CHtmlStyle();
            ++visitor;
    ...
    #region 分割 Token
        ...
        private void ParseTokens()
        {
            ...
            while(index < m_input.Length)
            {
                ...
                if(index == newIndex) // Text
                {
                    ...
                    if(m_input[index].Equals('<')) // 已知目前 '<' 為文字的並非 tag ?頭, 找尋其他 tag 的 '<' ?頭
                    {
                        if(index + 1 < m_input.Length)
                            textEndIndex = m_input.IndexOf("<", index + 1);
                        else textEndIndex = -1;
                    }
                    else textEndIndex = m_input.IndexOf("<", index); // 找尋 tag 的 '<' ?頭
                    ...
                    else // 沒有找到其他 tag 將所有剩下的內視為 Text ?就是最後?? text tag
                    {
        ...
        private int ParseStartTagTokens(int index, List<Token> tokens)
        {
            ...
            {
                // ?始條件為 "<"
                if(index >= m_input.Length) throw new ParseExeception(); // 意?結?
                if(m_input[index].Equals('<') == false) throw new ParseExeception(); // 最前?必須是 '<'
                index = index + 1;
                if(index >= m_input.Length) throw new ParseExeception(); // 意?結? 
                ...
                {
                    // 取得 tag 名稱
                    if(CHtmlUtil.EqualesOfAnyChar(m_input[index], CHtmlUtil.WhiteSpaceChars + "\'\"=/<?")) throw new ParseExeception(); // 第??字元不可以是泛型空白或是 '\'' '\"' '=' '/' '<' '?'
                    int tag_name_start = index++;
                    while(index < m_input.Length && !CHtmlUtil.EqualesOfAnyChar(m_input[index], CHtmlUtil.WhiteSpaceChars + "/>")) index = index + 1;  // 找到結尾 泛型空白字元 與 '/' '>'
                    if(index >= m_input.Length) throw new ParseExeception(); // 意?結?
                    int tag_name_end = index - 1;
                    string tagName = m_input.Substring(tag_name_start, tag_name_end - tag_name_start + 1).ToLower();

                    while(index < m_input.Length && CHtmlUtil.IsWhiteSpaceChar(m_input[index])) index = index + 1;  // 跳過泛型空白字元
                    if(index >= m_input.Length) throw new ParseExeception(); // 意?結? 
        ...
        private int ParseNoEscapingTagTokens(int index, List<Token> tokens, string tagName)
        {
            ...
            try
            {
                if(index < m_input.Length)
                {
                    ...
                    while(noEscapingEndIndex != -1) // No Escaping End Tag 邊?條件不同於??的 Tag
                    {
                        // Read tag name
                        int tag_name_start = noEscapingEndIndex + 2;
                        int tag_name_end = tag_name_start;
                        while(tag_name_end < m_input.Length && !CHtmlUtil.EqualesOfAnyChar(m_input[tag_name_end], CHtmlUtil.WhiteSpaceChars + "/>")) tag_name_end = tag_name_end + 1;  // 找到結尾 泛型空白字元 與 '/' '>'
                    ...
                    if(noEscapingEndIndex == -1) // 沒有找到 End Tag 將所有剩下的內視為 no escaping tag 的?部份
                    {
        ...
        private int ParseEndTagTokens(int index, List<Token> tokens)
        {
            ...
            try
            {
                // ?始條件為 "</"
                if(index >= m_input.Length) throw new ParseExeception(); // 意?結?
                if(index + 1 < m_input.Length && m_input.IndexOf("</", index, 2) == -1) throw new ParseExeception(); // 最前?必須是 '</'                
                index = index + 2;
                if(index >= m_input.Length) throw new ParseExeception(); // 意?結? 
        ...
        private int ParseCommentTagTokens(int index, List<Token> tokens)
        {
            ...
            try
            {
                // ?始條件為 "<!--"
                if(index >= m_input.Length) throw new ParseExeception(); // 意?結?
                if(index + 3 < m_input.Length && m_input.IndexOf("<!--", index, 4) == -1) throw new ParseExeception(); // 最前?必須是 '<!--'
                index = index + 4;
        ...
        private int ParseAttributeTokens(int index, List<Token> tokens)
        {
            ...
            try
            {
                ...
                {
                    // 邊?條件是 ">" 或是 "/>" 字?
                    while(index < m_input.Length && CHtmlUtil.IsWhiteSpaceChar(m_input[index])) ++index; // 跳過泛型空白字元
                    if(index >= m_input.Length) throw new ParseExeception(); // 意?結?                    
                    ...
                    else if(CHtmlUtil.IsVaildAttribueNameChar(m_input[index]) == false) // 意?出? '\'' '\"' 字元 跳過
                    {
                        ++index;
                        continue;
                    }
                    ...
                    while(index < m_input.Length && CHtmlUtil.IsVaildAttribueNameChar(m_input[index])) index = index + 1;  // 找到結尾 泛型空白字元 與 '/' '<' '>' '=' '\'' '\"'
                    if(index >= m_input.Length) throw new ParseExeception(); // 意?結?                    
                    ...
                    if(m_input[index].Equals('=')) // if the attribute is not null-attribute
                    {
                        ...
                        if(m_input[index].Equals('\"') || m_input[index].Equals('\'')) // 以 "xxx" 'xxx' ?覆的屬性?
                        {
                            ...
                            if(index == -1) throw new ParseExeception(); // 意?結?     
                        ...
                        else
                        {
                            // get value
                            value_start = index;
                            while(index < m_input.Length && CHtmlUtil.IsVaildAttribueValueChar(m_input[index])) ++index; // 找尋屬性?結尾
                            if(index >= m_input.Length) throw new ParseExeception();// 意?結?                                                            
                            value_end = index - 1;
                        }
	...
	#region	詹幗滲熱
        ...
        /// 
        /// </summary>
        private CHtmlNodeCollection m_result = null;
        /// <summary>
        /// 
        /// </summary>
        ...
        /// 
        /// </summary>
        private CHtmlNodeCollection m_currentLevel = null;
